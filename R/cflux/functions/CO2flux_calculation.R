...
input <- sites.data.2017Li1400[[800]]
input <- sites.data.2017SQ[[3]]
...
fluxcalc<-function(input){
  ...
  h2O<-input$dat$H2O # mean H2O value for measurement mmol/mol
...
# Jasoni et al 2005 Net ecosystem CO2 exchange in Mojavo desert shrublands during 8 year of exposure to elevated CO2
  ...
  nee<- -(vol*press*(1000-mean(h2O))*dcdt)/ (R*area*(temp+273.15))# D(Reco)= negative L(NEE)=positive
